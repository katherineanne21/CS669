...
SELECT (price_in_us_dollars *
	   ...
	   WHERE currency_id = 
			...
			FROM Store_location
			WHERE store_name = 'London Extension'))) AS price_in_euros
FROM Product
...
SELECT product_name, to_char(price_in_us_dollars *
	   ...
	   WHERE currency_id = 
	   		(SELECT currency_accepted_id
			FROM Store_location
			WHERE store_name = 'London Extension')), 'FM€999999.00') AS price_in_euros
FROM Product
WHERE (price_in_us_dollars *
	  (SELECT us_dollars_to_currency_ratio
	   ...
	   WHERE currency_id = 
	   		(SELECT currency_accepted_id
			FROM Store_location
			WHERE store_name = 'London Extension'))) < 26 
		OR 
		(price_in_us_dollars *
	  	(SELECT us_dollars_to_currency_ratio
	  	 FROM Currency
	  	 WHERE currency_id = 
	   		(SELECT currency_accepted_id
			FROM Store_location
			WHERE store_name = 'London Extension'))) > 299;
...
FROM (SELECT Sells.product_id
	 FROM Sells
	 GROUP BY Sells.product_id
	 HAVING COUNT(DISTINCT Sells.store_location_id) = (SELECT COUNT(*) FROM Store_location)) locations
JOIN Product ON Product.product_id = locations.product_id
JOIN Alternate_name ON Alternate_name.product_id = locations.product_id
...
WHERE EXISTS (SELECT 1
        	  FROM Sells s
			  JOIN Store_location ON Store_location.store_location_id = s.store_location_id
        	  WHERE s.product_id = Product.product_id
			  GROUP BY s.product_id
			  HAVING COUNT(DISTINCT s.store_location_id) = (SELECT COUNT(*) FROM Store_location));
...
SELECT All_locations.product_name, Alternate_name.name
FROM All_locations
JOIN Alternate_name ON Alternate_name.product_id = All_locations.product_id;
